  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await axios.post(
        "https://motofix-server.vercel.app/api/chatbot",
        { question }
      );
      ...
      <div className="bg-gray-900 w-full max-w-md min-h-96 md:min-h-112 md:min-w-4xl rounded-xl shadow-lg flex flex-col">
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-3 max-h-100">
          {messages.map((msg, idx) => (
            <div
              key={idx}
              className={`p-2 rounded-lg max-w-[80%] wrap-break-word ${
                msg.type === "user"
                  ? "bg-white self-end text-white"
                  : "bg-gray-100  self-start text-white"
              }`}
            >
              {msg.text}
            </div>
